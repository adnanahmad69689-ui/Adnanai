# Hero Image Update Checklist

- [x] Copy the supplied `hero.webp` into persistent web asset storage and obtain its project URL.
...
# Identity and Hero Refinement Checklist

- [x] Standardize visible personal-name copy to Adnan Ai.
...
# Independent Infrastructure Migration Checklist
...
- [x] Restore direct Cloudflare `/admin` routing to the Supabase owner sign-in gate without affecting public navigation.
...
# Hero Greeting-to-Logo Replacement Checklist
...
# Complete Pre-Push Mobile Audit Checklist
- [x] Audit every public route, navigation path, footer link, form, component state, and responsive layout at 320px, 360px, 375px, 390px, 414px, 430px, 768px, 1024px, and desktop. Full-page reviews completed at every required width plus 1280px; all public sections, project cards, AI System cards, Contact form, images, and footer content remained visible with no horizontal overflow observed. The `#n8n-projects` public workflow route rendered fully at 390px and 1280px, while `/admin`, `/reset-password`, and an unknown route rendered intentional nonblank states at 390px and 1280px.
- [x] Restore visible `.reveal-item` styling after the existing narrow-screen fallback adds `.is-revealed`; the audit confirmed the CSS base opacity remained `0`, which could make loaded mobile content transparent outside screenshot capture. The compact breakpoint now applies the same visible state to any late-rendered reveal item.
- [x] Ensure reveal observation also covers asynchronously rendered Website and AI System cards on desktop; the DOM audit confirmed those deferred cards retained base opacity `0` after loading because they were not present during the initial observer setup. The existing observer now registers added reveal items without changing the desktop animation behavior.
- [x] Fix only verified blank-page, rendering, overflow, navigation, hidden-content, animation, image, or responsive issues without removing content or altering desktop design. Only the two verified reveal-state defects were corrected.
- [x] Test all routes, navigation, footer, forms, buttons, images, animations, console health, horizontal overflow, desktop, tablet, and mobile after any correction. DOM checks confirmed zero horizontal overflow, no incomplete page images, and no transparent reveal items at every required width; mobile and desktop navigation targets worked, the mobile menu opened/closed after selection, the Contact Service control opened/closed with its label, blank form validation worked, no form was submitted, and no critical console errors were captured. Footer intentionally renders no links.
- [x] Run the production build, review Git status for intended changes only, then commit, push, wait for deployment, and verify the real site. `pnpm check`, 38 Vitest tests, and `pnpm build:cloudflare` passed before commit `159912a`; it was pushed to `github/main`. Cloudflare then served the new reveal CSS and canonical `https://adnanai.com/` post-deployment DOM checks passed at 390px and 1280px with no overflow, hidden reveal items, broken images, or critical console errors.
